...
public class SubscribeHandler implements ButtonClickHandler {
    ...
    public Uni<Response> handleWithArgs(Long chatId, String... args) {
        return userRepository.findByChatId(chatId)
                .map(userDataDb -> {
                    userDataDb.setSubscribed(!userDataDb.getSubscribed());
                    return userDataDb;
                })
                .map(userDataDb -> {
                    userRepository.persistOrUpdate(userDataDb);
                    return userDataDb;
                })
